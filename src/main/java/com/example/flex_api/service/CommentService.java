...
import com.example.flex_api.repository.ArticleRepository;
import com.example.flex_api.repository.CommentRepository;
...
public class CommentService {
    ...
    public CommentResponseDto createComment(Long id, String text){
        Member member = memberRepository.findById(
                ...
                .orElseThrow(() -> new RuntimeException("고그인 유저 정보가 없습니다."));
        Article article = articleRepository.findById(id).orElseThrow(()->new RuntimeException("댓글이 없습니다."));
    ...
    public void removeComment(Long id) {
        ...
        if(!comment.getMember().equals(member)) {
            throw new RuntimeException("작성자의 로그인이 일치하지 않습니다.");
        }
